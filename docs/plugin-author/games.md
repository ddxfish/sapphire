...
- **`event`** games poke moments: `ctx.poke("Wave 12 cleared. Castle 40/100.")`. She gets a turn no sooner than the min gap after her last one. The note is what she reads. When your moments come faster than she can answer, declare `cadence_every` in `room_defaults` (Dark Horse says 3): only every Nth poke becomes her turn, and the room's pill shows the count. `ctx.poke(note, {force: true})` marks a terminal moment (the castle fell, the run ended) that always comes through.
