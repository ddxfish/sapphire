...
| `room_defaults` | No | Your game's own defaults for the room settings spine, `{key: value}` — its nature: `cadence_mode` (`off` \| `turn` \| `event` \| `timer`), `cadence_every`, `send_frames`, a timer range… Sits between the player's room defaults and their per-game overrides (see below) |
| `cadence_event` | No | One word for your game's moment — `wave`, `hand`. The every-N row reads "every N waves" and the room's pill "her turn every 3rd wave". Only meaningful with `cadence_mode: event` |
...
Every key is declared once (`gameroom_core.ROOM_KEYS`) with two independent axes: `scope`, the layers allowed to hold a value, and `show_in`, the sidebars that render it (`library` = the Game Room's own sidebar, `room` = inside a game; absent = both). The birth toolset (`new_session_toolset`) shows in the library only — inside a running game it could only affect the next session. The user's per-game layer accepts only keys shown in rooms. Today's keys: `player_name`, `llm_primary`, `return_prompt` (room only); `cadence_mode` — the nature: `off` (no unprompted turns), `turn` (she speaks only in reply to moves and messages through her seat — poker), `event` (the game posts moments, every Nth is her turn — a wave game), `timer` (a random clock between min and max — Doom, a movie); one lever per nature: `cadence_every` for events (game + session, room-shown only — it sits with the nature; worded with the game's `cadence_event` noun), `cadence_min` / `cadence_max` (seconds, timer only); `cadence_paused` (session only); `send_frames` (show her the screen each turn) with `frames_per_tick` and `frame_short_edge_px`; `tts_route` (`browser` | `speakers`); `session_end_summary`; `session_prompt_piece` (a costume line injected on the system prompt of this game's sessions; room, game); `new_session_toolset` (birth default; room, game).
...
- Players edit the room layer in the library sidebar (accordions Room / Cadence / Voice / Identity, pre-filled with the shipped defaults) and a game's layer in the same accordions inside that game's room sidebar, pre-filled with what the game inherits (the Voice rows join the room's own TTS accordion rather than making a second one). An edited value shows a dot and a ↺ that inherits again; typing the inherited value back is not an override. The session layer has no fields — the room's pause control writes `cadence_paused`.
- Your engine's own `SETTINGS` keys never collide with room keys — game overrides are stored beside them under `_room`.
- Read the resolved values from `GET play/room/effective?game=<id>&session=<chat>` (with `layers` naming which layer set each key), or in-process `gameroom_core.effective(game_id, session)` / `effective_for_chat(chat)`.

## Her unprompted turns — cadence and perception

Inside a game room the host arms core's **cadence organ** for the session from the resolved spine (`cadence_mode`, the gap, `send_frames`, `tts_route`) and keeps it alive while the room is open. Each unprompted turn is a real turn on the session chat through the same door the wake word uses: it streams into the room's rail, the subtitle carries it, Stop works, and her voice plays where `tts_route` says.

- **`turn`** games never arm — she speaks in reply to moves and messages (your seat + the chat).
- **`event`** games poke moments: `ctx.poke("Wave 12 cleared. Castle 40/100.")`. She gets a turn no sooner than the min gap after her last one. The note is what she reads. When your moments come faster than she can answer, declare `cadence_every` in `room_defaults` (Dark Horse says 3): only every Nth poke becomes her turn, and the room's pill shows the count. Event mode has no min/max gap — every-N is its one lever (the organ keeps a fixed breath between turns). Games that don't use her declare `cadence_mode: off`. `ctx.poke(note, {force: true})` marks a terminal moment (the castle fell, the run ended) that always comes through.
- **`timer`** games get a random gap in [min, max]; the host feeds her frames of a `<canvas>` in your stage automatically while `send_frames` is on (scaled to `frame_short_edge_px`, the last `frames_per_tick` kept). A board can deposit its own view instead: `ctx.deposit({ frames: [{data, media_type}], text })` — latest wins, taken at fire time, never persisted (the row keeps the words).
- Her own live turn on the chat = the tick is skipped and the gap stretches; the ⏸ in the stage bar writes the session's `cadence_paused`.
- Leaving the room disarms and, when `session_end_summary` is on, runs her summary turn ("the session is ending — write it up in your own words") after you've gone.

Depositors outside the browser use the core door: `POST /api/perception/{chat}` with `{frames, text, source}`; `GET /api/cadence/{chat}` reports the organ's state for a chat.
