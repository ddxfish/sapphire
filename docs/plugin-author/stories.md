...
- **Takeable objects**: `"takeable": true` lets the generic `take` verb (grab/get/pocket aliases) move the object itself into inventory — journaled `taken` event, the object leaves the room (visibility, search, and her room text all exclude it). Optional `take_message`. A declared `take` interaction on a takeable object **composes** with the pocket (2026-08-23): its message, effects, dice or seal fire *and* the object is taken — "what she says when it's picked up" is just a `take` action card. Non-takeable objects keep the off-script narrate-freely license.
- **Conditions** also accept `{"did": "door1"}` (the object has been used — replayed interaction history, verb-agnostic) and `{"solved": "vault"}` (its puzzle is answered) — no flag wiring needed for has-opened / password gates. The in-app editor compiles to exactly this grammar, **action-first** (2026-08-21): the object owns its description, **Can be picked up**, **Visible when** (top-level `condition` + `hidden` for search-reveal) and **Riddle** (`puzzle` + solve effects → `on_solve`); each *action card* is one verb with its **own** Requirements (condition + blocked_message, `{solved}` via the 'riddle solved' row, `roll` for d20/d100 chances) and Effects (`set`/`adjust`/`gives`/prompt pieces/`goto`) — per-verb divergence is native, matching the engine. Since 2026-08-23 the card's verb field is **CSV** (`open, unlock, force` → canonical + `aliases`), the riddle answer field is CSV (`map, an atlas` → `solutions`), the dice row carries the miss line (`failure.message`) and a **one try** box (`once`), and both condition panels speak `number at least` (`flag_gte`) and `after turns in this room` (`after_turns`). Rooms get an **On entering** effects block (`on_enter`, stored wholesale on the playthrough layer).
...
- **Shipped OBJECT mechanics edit through the same gate** (2026-08-21): an object whose spec round-trips through the editor losslessly prefills live; a save stores a `_replace` entry that swaps your object **wholesale** for that playthrough (verbatim-equals-shipped clears it; restore lifts it). Fields the editor doesn't show — per-verb `aliases`, `emotions`/`emotions_remove`, roll `once`/`retry_message`/`failure` branches, `found_by`/`gives`, `on_solve` message/emotions — are *passengers*: they ride a gated save verbatim, attached to their carrier (delete the verb or its dice and they go with it). Per-verb-divergent locks now round-trip natively (action-first form, 2026-08-21); `sealed` interactions edit live too (2026-08-23) — the **ask the player (popup)** Effects row is `sealed.ask` + `sealed.fallback`, the action's response box is `sealed.hold_message`, `wait` rides as a passenger; a seal and a dice roll never share a verb (the engine answers at the seal). Multi-answer `solutions` and a seal with extra keys still fail the gate and show read-only in plain words; text rewording still shadows those, machinery riding untouched.
...
**Conditions** (on exits, interactions, blockers, ending cards): `{"has": "item"}`, `{"flag": "name"}` (truthy), `{"flags": {"k": expected}}`, `{"flag_gte": {"k": 50}}` (numeric threshold), `{"after_turns": 3}` (the player has been in the *current* room that many turns — a guard who appears after three, a hint object that surfaces when they're stuck) — all listed clauses must hold.
...
STORY PACKS (Game Room story engine):
- A story pack = any plugin with a `stories/<slug>/` dir: story.json + rooms/{id}-{slug}.json + backdrops/ (+ optional prompts/pieces.json). No manifest capability needed — the host scans every plugin's stories/ at read time. Backdrops serve through an images-only lane; room JSONs (solutions) are never web-served.
- Law: the AI narrates, the referee rules — solutions/hidden objects/sealed text live engine-side and never enter her context.
- story.json: slug (stable id; leading `_` hides from picker), title, description, start (required), premise?, role? {name, text} (enables story mode = full identity swap), player_role?, dm_guide? (user-editable default), initial_flags?, tags/facts/tile?, open_flag? (the zork-line: player room-text overrides apply when it turns truthy), slots? (Mad-Libs setup form), start_items? ({name: spec} — pocket objects, no hidden/condition/takeable, membership derived at load, keys anchor room-independently).
- Slots: {key, label, options?, allow_custom?, default?, rows?, section?, width?, "label": "" allowed}; {key} tokens substitute once at load into role/premise/player_role/dm_guide/all room strings. sealed:true + seal_key "{room_id}:{object}:{verb}" writes the player's text into that sealed blank instead of substituting. Role of "{slot}" tokens = assembled-setup-as-role pattern (give solid defaults).
- Room JSON: id, slug, title, template (AI-side, spoiler-bearing), player_desc (player-side, spoiler-free), player_hints [{after_turns, text}] (click-to-reveal), ghost_hints [{after_turns, whisper}], on_enter (effects), exits, objects, blockers [{until, text}], backdrop, ending_cards [{card, condition}] (first match wins). No exits = ending.
- Exits: to, label, desc, condition + blocked_message (visible but refusing), visible_when (existence gate), effects (fire on traverse, before destination on_enter), roll (dice-gated; only success moves; journaled). No puzzle on exits — use a door object + {"did"/"solved": id}.
- Objects: desc, hidden + found_by "search", takeable (+take_message; a declared take interaction COMPOSES with the pocket), gives, puzzle {riddle, solutions (forgiving match)}, on_solve, top-level condition (object doesn't exist until it holds), interactions {verb: {aliases, condition, blocked_message, message, set/adjust/gives/emotions/extras/goto/show, roll, sealed}}. A declared `look` interaction replaces the generic read (examine/inspect route into it). Off-script verbs return an improvisation license, never a refusal. Carried objects stay fully interactable anywhere.
- Effects: set {flag: value}, adjust {flag: +n}, gives item, emotions/emotions_remove, extras/extras_remove (prompt pieces), goto room_id (no on_enter fire), show image lightbox (player-only; referee tells her one fired).
- Conditions (AND of all clauses): {has}, {flag} (truthy), {flags {k: expected}}, {flag_gte {k: n}}, {after_turns n} (turns in current room), {did object}, {solved object}.
- Dice: roll {sides, beat, once, success {effects}, failure {message}, retry_message} — rolled value journaled at roll time (replay never re-rolls).
- Sealed blanks: interaction carries sealed {ask, fallback?, hold_message?, wait?} — player writes the reveal in a popup raised ONLY when she reaches for the blank (live countdown default 120s, per-seal wait or meta.seal_wait, clamp 5-600); unfilled/no-player => honest hold; fallback only on explicit player choice; fills are journal events; effects fire once at first reveal; a seal and a dice roll never share a verb.
- Identity: role shipped => default story mode (full swap, prompt registers under role name); local = her own persona; combined = both. Byte-exact prompt preview (eye icon); no hidden instructions. GM law "Max one move per turn" default on.
- Emotion pieces: prompts/pieces.json keys `story_<slug>_<emotion>`; resolution: story-specific, then story_engine_<emotion>, else skipped.
- Saves: the chat IS the save — per-(story, chat) event journal on the chat; state = pure replay (no LLM/randomness/clock). Journal follows the chat: rename/private-encrypt/delete. Vault-locked chat: playthrough invisible, engine refuses writes, role prompt name hidden. In local/combined the local persona can't be a vault prompt.
- Player-side editing (scenarios/shadows) is user-layer only: shipped-object rewording field-merges (mechanics survive), mechanics edits go through fidelity gates (_replace / mechanics units, wholesale swap per playthrough); the pack stays canonical. Ship nothing for any of it.
- Checklist: reachable rooms, start exists, tile/backdrops ~q82 JPG, sign plugin, 📖 tile in Game Room library.
